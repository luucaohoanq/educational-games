...
export default function GameList({ username, role }: GameListProps) {
  ...
  useEffect(() => {
    const preventDefaultScroll = (e: KeyboardEvent) => {
      if (selectedGame && ["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Space"].includes(e.code)) {
        e.preventDefault();
      }
    };
          ...
          <div className="grid lg:grid-cols-[minmax(0,960px)_380px] gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-lg border border-[#d9e3f1] shadow-sm p-5">
                <h3 className="text-sm font-bold text-[#575e75] mb-3">Project Stats</h3>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-[#8fa1b3]">Views</span>
                    <span className="text-sm font-bold text-[#575e75]">{detailGame.views || 0}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-[#8fa1b3]">Loves</span>
                    <span className="text-sm font-bold text-[#575e75]">{detailGame.likes || 0}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-[#8fa1b3]">Date Shared</span>
                    <span className="text-sm font-bold text-[#575e75]">
                      {detailGame.dateAdded 
                        ? new Date(detailGame.dateAdded).toLocaleDateString('en-US', { 
                            month: 'short', 
                            day: 'numeric', 
                            year: 'numeric' 
                          })
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-lg border border-[#d9e3f1] shadow-sm p-6">
                <h2 className="text-lg font-bold text-[#575e75] mb-3">Instructions</h2>
                <div className="text-[#575e75] text-sm leading-relaxed whitespace-pre-wrap">
                  {detailGame.description || detailGame.instructions || 
          ...
          <div className="mt-6 grid lg:grid-cols-[minmax(0,900px)_260px] gap-6">
              ...
              <div className="bg-white rounded-lg border border-[#d9e3f1] shadow-sm p-6">
                ...
                <div className="space-y-4">
                  {topLevelComments.map((comment) => (
                    <div key={comment.id} className="border-l-4 border-[#4c97ff] pl-4">
                      <div className="bg-[#f7f9fc] rounded-lg p-4">
                        <div className="flex items-start gap-3 mb-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <p className="font-bold text-[#575e75] text-sm">{comment.username}</p>
                              <span className="text-xs text-[#8fa1b3]">
                                {new Date(comment.datePosted).toLocaleDateString()}
                              </span>
                            </div>
                            <p className="text-[#575e75] text-sm leading-relaxed">{comment.content}</p>
                        ...
                        {getReplies(comment.id).map((reply) => (
                          <div key={reply.id} className="mt-3 ml-13 bg-white rounded-lg p-3 border border-[#d9e3f1]">
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                  <p className="font-bold text-[#575e75] text-xs">{reply.username}</p>
                                  <span className="text-xs text-[#8fa1b3]">
                                    {new Date(reply.datePosted).toLocaleDateString()}
                                  </span>
                                </div>
                                <p className="text-[#575e75] text-sm leading-relaxed">{reply.content}</p>
                              </div>
                            </div>
                          </div>
                        ))}
                  ...
                  {comments.length === 0 && (
                    <div className="text-center py-12">
                      <p className="text-[#8fa1b3] text-sm">No comments yet</p>
                      <p className="text-[#8fa1b3] text-xs mt-1">Be the first to share what you think!</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className=" p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold text-[#575e75]">More Games</h3>
      ...
      <div className="max-w-[1280px] mx-auto px-6 py-8">
            ...
            <form onSubmit={handleUpload} className="space-y-5">
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  />
                  {thumbnailFile && (
                    <p className="text-xs text-[#0fbd8c]">✓ {thumbnailFile.name}</p>
                  )}
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    {file ? (
                      <>
                        ...
                        <p className="text-[#0fbd8c] font-bold text-sm">{file.name}</p>
                      ...
                      <>
                        ...
                        <div>
                          <p className="text-[#575e75] font-bold mb-2 text-sm">
                            {isDragging ? "Drop your file here" : "Drag and drop or"}
                          </p>
                          <label
                            htmlFor="file-upload"
        ...
        {games.length > 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-[#575e75] font-bold text-lg mb-2">No projects yet</p>
            <p className="text-[#8fa1b3] text-sm">
              {username ? "Create your first project to get started!" : "Sign in to create projects"}
            </p>
          </div>
        )}
      </div>
